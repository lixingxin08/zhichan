<template>
  <div class="administrativedivision flex_fs">
    <div class="isleft">
      <is-left-tree v-if="showTree" :treedata="treedata" @parentdata='parentdata' :defaultExpandedKeys="defaultExpandedKeys"
    ...
    <div class="content2">
      <div class='flexrow flexac flexsb' style="margin-bottom: 20px;">
        <div class="flexrow flexac">
          <div class='title_tx'>名称/分机号:</div>
          <div style="width: 200px;">
            <a-input placeholder="请输入名称/分机号" v-model="keyword" />
          </div>


          <div class='title_tx' style="margin-left: 20px;">求助装置状态:</div>
      ...
      <a-table :scroll="{  y: 700 }" :columns="tableTitle" :data-source="tableData" bordered size="small" :pagination="pagination"
        ...
        <div slot='lightPoleName'>{{isselectdata.name}}</div>
  ...
  export default {
    ...
    methods: {
      ...
      async getTableData() {
        ...
        let res = await this.$http.post(this.$api.devicepolicepage, param)
      ...
      async confirmDelete(item) {
        ...
        let res = await this.$http.post(this.$api.devicepoliceremove, param)
        if (res.data.resultCode == 10000) {
          this.getTableData()
