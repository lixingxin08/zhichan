<template>
  <div class="content2">
    <div style="margin: 0 auto;">
      ...
      <div class="flexrow flexac edit_item_ko_first">
        <div class="edit_item_title_ko_first"><span style="color: #FF0000;">*</span>发布屏型号:</div>
      ...
      <div class="flexrow flexac edit_item_ko_first">
        <div class="edit_item_title_ko_first"><span style="color: #FF0000;">*</span>发布屏名称:</div>
        <div class="edit_item_input">
          <a-input v-model="config.deviceName" :maxLength='30' placeholder='30字以内，中文汉字、英文字母、数字、英文下划线、中英文小括号' />
        </div>
      </div>
      <div class="flexrow flexac edit_item_ko_first">
        <div class="edit_item_title_ko_first"><span style="color: #FF0000;">*</span>发布屏编号:</div>
        <div class="edit_item_input">
      ...
      <div class="flexrow flexac edit_item_ko_first">
        <div class="edit_item_title_ko_first"><span style="color: #FF0000;">*</span>发布屏状态:</div>
    ...
    methods: {
      ...
      async submit() {
        if (!this.config.modelId) {
          this.$message.warning('请选择发布屏型号')
          return
        }
        if (!this.config.deviceName) {
          this.$message.warning('请填写发布屏名称')
          return
        }
        if (!this.$utils.vify_cn30(this.config.deviceName)) {
          this.$message.warning('发布屏名称请输入30字以内，中文汉字、英文字母、数字、英文下划线、中英文小括号')
          return
        }
        if (!this.config.deviceCode) {
          this.$message.warning('请填写发布屏编号')
          return
        }
        if (!this.$utils.vify_cn30(this.config.deviceName)) {
          this.$message.warning('发布屏编号请输入30字以内，中文汉字、英文字母、数字、英文下划线、中英文小括号')
          return
        }
        if (this.config.statusCode < 0) {
          this.$message.warning('请选择发布屏状态')
          return
        }
        this.config.poleId = this.lightConfig.id
        let res = await this.$http.post(this.$api.devicemultimediaform, this.config)
        if (res.data.resultCode == 10000) {
          this.$message.success(res.data.resultMsg)
            this.$router.push("/layarled")
        } else {
      ...
      async getDetail() {
        ...
        let res = await this.$http.post(this.$api.devicemultimediadetail, param)
        if (res.data.resultCode == 10000) {
